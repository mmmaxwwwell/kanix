...
- When `PaymentAdapter` interface gains a new method (e.g. `submitDisputeEvidence`), every test file with a local `createStubPaymentAdapter()` function must be updated — there are 20+ copies across integration tests. Consider extracting a shared `createStubPaymentAdapter` from `test-helpers.ts` in a future cleanup.
- `redactCustomerPII<T extends Record<string, unknown>>` is too restrictive — typed interfaces like `CustomerListItem`/`CustomerDetail` lack index signatures. Use specific field constraints instead of `Record<string, unknown>`.
- `WsMessage` has `type/entity/entityId/data/sequenceId`; buffer entry timestamps live on the buffer wrapper (`.timestamp`), not on the message itself.
- `fulfillment-propagation.integration.test.ts` used hardcoded variant UUIDs (`00000000-…-000001`) that don't exist in `product_variant` — FK constraint `fk_order_line_variant` rejects every insert. Fix: create real product+variant rows in `beforeAll`.
